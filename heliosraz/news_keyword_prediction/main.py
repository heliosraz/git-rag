...
def data_split(X: list[defaultdict], y: list[list[int]]):
    ...
    X, X_test, y, y_test = sklearn.model_selection.train_test_split(
        X, y, train_size=0.90, test_size=0.10
    )
    X_train, X_dev, y_train, y_dev = sklearn.model_selection.train_test_split(
        X, y, train_size=0.88, test_size=0.12
    )
...
# Splitting the data
# Unigrams
X_train_uni, y_train_uni, X_dev_uni, y_dev_uni, X_test_uni, y_test_uni = data_split(
    unigrams, true_labels_unigrams
)
num_entries_uni = len(y_test_uni)

# Bigrams
X_train_bi, y_train_bi, X_dev_bi, y_dev_bi, X_test_bi, y_test_bi = data_split(
    bigrams, true_labels_bigrams
)
num_entries_bi = len(y_test_bi)


# Trigrams
X_train_tri, y_train_tri, X_dev_tri, y_dev_tri, X_test_tri, y_test_tri = data_split(
    trigrams, true_labels_trigrams
)
...
for solver in ["lbfgs", "liblinear", "newton-cg", "sag", "saga"]:
    for C in [100, 10, 1.0, 0.1, 0.01]:
        ...
        logistic_uni[solver, C] = accuracy(
            prediction_log_uni, y_dev_uni, num_entries_uni, 1
        )
        print(solver, C, accuracy(prediction_log_uni, y_dev_uni, num_entries_uni, 1))
        logistic_bi[solver, C] = accuracy(
            prediction_log_bi, y_dev_bi, num_entries_bi, 1
        )
        print(solver, C, accuracy(prediction_log_bi, y_dev_bi, num_entries_bi, 1))
        logistic_tri[solver, C] = accuracy(
            prediction_log_tri, y_dev_tri, num_entries_tri, 1
        )
...
for feat in [10, 100, 1000]:
    for n in range(1, 20 + 1):
        ...
        forest_uni[feat, n] = accuracy(
            prediction_forest_uni, y_dev_uni, num_entries_uni, 1
        )
        ...
        forest_tri[feat, n] = accuracy(
            prediction_forest_tri, y_dev_tri, num_entries_tri, 1
        )
...
params = {
    "max_features": forest_max_hyper_uni[0],
    "n_estimators": forest_max_hyper_uni[1],
}
