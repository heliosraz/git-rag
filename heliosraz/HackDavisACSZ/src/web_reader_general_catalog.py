...
for href in major_hrefs:
    if "graduate" not in href:
        ...
        class_entryinfos = major_soup.find_all(
            "div", class_="container-fluid course-summary-wrapper"
        )
        class_prereqinfos = major_soup.find_all(
            "div", class_="col-xs-12 col-sm-12 col-md-12 text-left full-width-column"
        )
        # initializing class name list
        for classentryinfo in class_entryinfos:
            class_temp = classentryinfo.find(
                "div", class_="col-xs-10 col-sm-10 col-md-10 text-left left-column"
            )
            class_names.append(class_temp.text)
        for prereqentry in class_prereqinfos:
            prereqentry = (
                str(prereqentry.find_all("span")[2])
                .replace("<span>", "")
                .replace("</span>", "")
            )
            print(prereqentry)
            ...
            for prereq in prereqentry.split(";"):
                prereqs = re.findall(r"[A-Z]{3} [0-9]{3}", prereq) + re.findall(
                    r"[A-Z]{3} [0-9]{3}[A-Z]", prereq
                )
                print(prereqs)
                if len(prereqs) > 1:
        ...
        for index in range(len(class_names)):
            class_data.write(
                ",".join([class_names[index], class_prereqs[index]]) + "\n"
            )
        class_data.close()
