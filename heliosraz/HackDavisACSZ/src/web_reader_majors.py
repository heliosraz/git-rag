...
for href in major_hrefs:
    ...
    if "graduate" not in href:
        ...
        class_entryinfos = major_soup.find_all(
            "div", class_="row course-row-core block-entry course-entry pdf-flex-nowrap"
        )
        ...
        if len(class_entryinfos) > 0:
            for classentryinfo in class_entryinfos:
                class_temp = classentryinfo.find(
                    "div",
                    class_="col-xs-2 col-sm-2 col-md-2 three-column left-column text-left text-start",
                )
                class_names.append(class_temp.text)
            class_data = open(
                os.path.dirname(__file__) + "/major_files/" + href + "_req.csv", "w+"
            )
